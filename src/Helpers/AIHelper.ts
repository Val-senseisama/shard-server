...
export interface QuestBriefInput {
  ...
  blockers?: string;
  /** A course, channel or programme they already follow. Free text; may be a URL. */
  aids?: string;
  skipped?: string[];
  /**
   * User asked for learning-material suggestions.
   *
   * Opt-in, and default off, because this is the expensive path: resolving a
   * suggestion to a real link costs 100 YouTube quota units against a 10k/day
   * budget (PLAN-intake.md §8.1). A toggle is the quota gate as much as it is a
   * preference — people who want suggestions ask, everyone else costs nothing.
   *
   * Today it only makes the model emit a `searchHint` per mini-quest: what to
   * look for, never a URL. That is phase 1 of the two-phase design in
   * PLAN-intake.md §8.2, and it costs no quota at all. Phase 2 — the server
   * resolving those hints into real videos — upgrades this field's meaning
...
LEARNING MATERIAL (only when the user asked for suggestions):
- Add a "searchHint" to each mini-quest: a short phrase someone would actually type into YouTube or Google to find material for it, e.g. "react hooks tutorial for beginners".
- NEVER output a URL, video id, channel name or book ISBN. You cannot verify they exist, and a dead link in someone's plan is worse than no link. Describe what to search for; something else resolves it.
- Omit "searchHint" entirely for any mini-quest that is doing rather than learning ("email three former colleagues"). A hint on every step trains people to ignore all of them.
...
/**
 * The user-side prompt for a quest breakdown.
 *
 * Exported so the streaming path sends *identical* instructions — two copies of
 * this string would drift, and a plan generated by the streaming path would
 * quietly differ from one generated by the fallback.
 */
...
- Workload preference: ${userContext.preferences.workloadLevel} (max ${userContext.preferences.maxTasksPerDay} tasks/day, preferred task length: ${userContext.preferences.preferredTaskDuration})`
  ...
  return `Goal: ${goal}${deadline ? `\nDeadline: ${deadline}` : ''}${userProfile}${formatBriefForPrompt(brief)}${brief?.wantsSuggestions ? '\n\nThe user asked for learning-material suggestions: include a "searchHint" on the mini-quests where material would genuinely help.' : ''}\n\nPlease break this down into a structured quest.`;
}

export { QUEST_ARCHITECT_PROMPT };
...
export async function breakDownGoalWithAI(
  ...
  const userPrompt = buildQuestUserPrompt(goal, deadline, userContext, brief);
 ...
 */
export async function generateReflectionMission(
  shardTitle: string,
  completionRate: number,
  /**
   * What the user said at intake. Optional so every existing call site keeps
   * compiling — and because it's genuinely absent on shards created before the
   * interview shipped.
   */
  brief?: QuestBriefInput
): Promise<{
...
Generate a short reflection side-quest to help them internalize what they learned.${formatBriefForPrompt(brief)}
If the section above says why this mattered to them, make one question ask directly about that — reflecting on the reason they gave beats a generic prompt.
...
export async function generateWeeklyTasks(
  ...
  weekNumber: number,
  /** The parent shard's brief, when the caller has loaded it. */
  brief?: QuestBriefInput
...
Context: ${miniGoalDescription || 'No additional context'}${formatBriefForPrompt(brief)}

Requirements:
- Each task should be specific and take 30-60 minutes
- If the section above gives the days and session length they actually have, size the tasks to fit ONE of those sessions rather than an ideal hour
